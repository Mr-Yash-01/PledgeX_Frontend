...
export default function SignUp() {
    ...
    useEffect(() => {
        ...
        if (emailInput) validateEmail({ target: emailInput } as React.ChangeEvent<HTMLInputElement>);
        if (passwordInput) validatePassword({ target: passwordInput } as React.ChangeEvent<HTMLInputElement>);
            ...
            .then(response => {
                try {
                    ...
                    if (response.status === 200) {
                        ...
                        user.picture = response.data.user.picture;
    ...
    const handleCreateAccount = () => {
        ...
        if (updatedUser.email && updatedUser.password && updatedUser.name && updatedUser.role) {
            axios.post('http://localhost:4000/auth/signup', updatedUser)
                .then(response => {
                ...
                <div className="w-full h-full flex flex-col gap-4 flex-grow rounded-lg px-6 py-4 shadow-2xl shadow-zinc-600 lg:flex-row ">
                    ...
                    <div className="flex flex-col items-center lg:w-1/2 lg:justify-center ">
                        ...
                        <div className="flex items-center justify-center gap-2 mt-2 lg:flex-col lg:gap-16 ">
                            <div id="step1Button" onClick={() => { handlestepButton(1); console.log('step1 tapped');
                             }} className={`cursor-pointer flex items-center justify-center w-16 h-16 rounded-full shadow-md shadow-gray-600 text-2xl underline-offset-1 ${currentStep === 1 ? 'underline underline-offset-4' : ''} `}>

                                {
                                    currentStep === 1 ? 1 : <IoMdCheckmark onClick={() => {console.log('checkmark tapped');
                                    }}></IoMdCheckmark>
                                }
                            </div>
                            <div className={`flex-grow h-1 w-28 border rounded-2xl bg-gradient-to-r ${isFillingBackground ? 'bg-fill' : ''} lg:rotate-90`}></div>
                            <div id='step2Button' onClick={() => handlestepButton(2)} className={`cursor-pointer flex items-center justify-center w-16 h-16 rounded-full shadow-md shadow-gray-600 text-2xl ${currentStep === 2 ? 'underline underline-offset-4' : ''} `}>
                                2
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Step 1 */}
                    <div id="step1" className={`mt-4 md:mt-0 lg:flex-grow lg:px-16 lg:py-4 ${currentStep === 1 ? 'block' : 'hidden'} lg:flex-col lg:justify-center`}>
                        ...
                        <div className="flex flex-row items-center justify-center gap-2 mt-4 lg:flex-col">
                            ...
                            <Info info="Sign In" link="/signin" className="text-center underline" />
                        </div>
                    </div>

                    {/* Step 2 */}
                    <div id="step2" className={`mt-4 md:mt-0 lg:flex-grow lg:px-16 lg:py-4 ${currentStep === 2 ? 'block' : 'hidden'} `}>
                        ...
                        <div className="flex flex-row gap-4 justify-around">
                            ...
                            <div className="flex flex-row justify-center items-center gap-2">
                                <Checkbox checked={iscb1Checked} id="cb1" onChange={() => {
                                    setcb1Checked(true);
                            ...
                            <div className="flex flex-row justify-center items-center gap-2">
                                <Checkbox checked={iscb2Checked} id="cb2" onChange={() => {
                                    setcb2Checked(true);
                                    setUser((prevState) => ({ ...prevState, role: 'Freelancers' }));
                                    setcb1Checked(false);                                   
                                }}></Checkbox>
